    ...
    >
        ...
        <div className="relative aspect-[3/4] w-full overflow-hidden rounded-sm bg-linen">
          ...
          <div className="absolute top-3 left-3 flex flex-col gap-1 z-10">
            {product.is_new_arrival && (
              <span className="bg-ink text-ivory text-[10px] tracking-[0.2em] uppercase px-2 py-1">
                New
              </span>
            )}
            {hasDiscount && (
              <span className="bg-saleTag text-ivory text-[10px] tracking-[0.2em] uppercase px-2 py-1">
                {off}% Off
              </span>
            )}
          </div>
          ...
          >
            <Heart
              size={16}
              strokeWidth={1.6}
              className={wished ? "text-rust fill-rust" : "text-ink"}
            />
          </button>
        </div>
      </Link>
      ...
      <div className="flex flex-col gap-1">
        ...
        <div className="flex items-center gap-2">
          ...
          {hasDiscount && (
            <span className="font-body text-sm text-smoke line-through">₹{product.price}</span>
          )}
        </div>
